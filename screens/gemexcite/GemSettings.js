...
import { User, Lock, Globe, Camera, Eye, EyeOff, CheckCircle, Building2 } from "lucide-react-native";
...
const GemSettings = () => {
    ...
    const prefill = async () => {
      try {
        ...
        if (raw) {
          ...
          setArea(userInfo?.profile?.area ?? "");
          setSourcingLocality(userInfo?.profile?.sourcingLocality ?? "");
        }
      } catch {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await api.put("/gem-excite/update-profile", {
        firstName,
        lastName,
        phoneNumber: phone,
        area,
      ...
      if (raw) {
        ...
        if (userInfo.profile) {
          userInfo.profile.area = area;
          userInfo.profile.sourcingLocality = sourcingLocality;
        }
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-gray-50">
      ...
      <View className="bg-white border-b border-gray-100">
        <View className="px-4 pt-2 pb-3">
          ...
          <Text className="text-[13px] text-gray-400 mt-0.5">{t("settingsSubtitle")}</Text>
        </View>
        <View className="flex-row px-4 gap-6">
          {[
            ...
            <TouchableOpacity
              ...
              style={{ borderBottomWidth: activeTab === key ? 2 : 0, borderBottomColor: "#A7CC48" }}
          ...
          {activeTab === TAB_PROFILE ? (
            <>
              ...
              <View className="items-center mb-6">
                <View className="relative">
                  <View
                    className="w-24 h-24 rounded-2xl bg-[#F0FDF4] border-4 border-white items-center justify-center"
                    style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 6 }}
                  >
                    <Building2 size={36} color="#A7CC48" />
                  </View>
                  <TouchableOpacity
                ...
                <Text className="text-[11px] text-gray-400 mt-2">{t("tapToUpdateClusterLogo")}</Text>
              </View>

              {/* First Name */}
              <View className="mb-4">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("firstName")}</Text>
                <TextInput
              ...
              {/* Last Name */}
              <View className="mb-4">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("lastName")}</Text>
                <TextInput
                  className="bg-white border border-gray-200 rounded-xl px-4 h-[50px] text-[13px] text-gray-800"
              ...
              {/* Email (disabled) */}
              <View className="mb-4">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("emailAddress")}</Text>
                ...
                <Text className="text-[11px] text-gray-400 mt-1 ml-1">{t("emailCannotChange")}</Text>
              </View>

              {/* Phone */}
              <View className="mb-4">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("phoneNumber")}</Text>
                <TextInput
                  className="bg-white border border-gray-200 rounded-xl px-4 h-[50px] text-[13px] text-gray-800"
              ...
              {/* Area */}
              <View className="mb-4">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("area")}</Text>
                <TextInput
                  className="bg-white border border-gray-200 rounded-xl px-4 h-[50px] text-[13px] text-gray-800"
                  placeholder={t("area")}
                  placeholderTextColor="#9CA3AF"
              ...
              {/* Sourcing Locality */}
              <View className="mb-6">
                <Text className="text-[13px] font-[600] text-gray-700 mb-1.5">{t("sourcingLocality")}</Text>
                <TextInput
                  className="bg-white border border-gray-200 rounded-xl px-4 h-[50px] text-[13px] text-gray-800"
                  placeholder={t("sourcingLocality")}
                  placeholderTextColor="#9CA3AF"
              ...
              {profileSaved && (
                <View className="flex-row items-center gap-2 bg-[#F0FDF4] border border-[#BBF7D0] rounded-xl px-4 py-3 mb-4">
                  ...
                  <Text className="text-[12px] font-[600] text-green-700">{t("profileUpdated")}</Text>
                </View>
              )}

              <TouchableOpacity
                className="rounded-2xl h-[52px] items-center justify-center"
                style={{ backgroundColor: isProfileValid ? "#A7CC48" : "#E5E7EB" }}
              ...
              >
                {profileLoading ? (
                  ...
                  <Text className="font-[700] text-[14px]" style={{ color: isProfileValid ? "#fff" : "#9CA3AF" }}>
                    {t("saveChanges")}
                  </Text>
                )}
              </TouchableOpacity>
            ...
            <>
              ...
              {pwdSaved && (
                <View className="flex-row items-center gap-2 bg-[#F0FDF4] border border-[#BBF7D0] rounded-xl px-4 py-3 mb-4">
                  <CheckCircle size={15} color="#22C55E" />
                  <Text className="text-[12px] font-[600] text-green-700">{t("passwordChanged")}</Text>
                </View>
              )}
              ...
              >
                <Text className="font-[700] text-[14px]" style={{ color: isPwdValid ? "#fff" : "#9CA3AF" }}>
                  {t("updatePassword")}
                </Text>
              </TouchableOpacity>
            </>
