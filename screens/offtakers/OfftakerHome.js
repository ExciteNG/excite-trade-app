...
const OfftakerHome = ({ navigation }) => {
  ...
  const renderOrder = ({ item }) => {
    ...
    const shortId = item.trackingId
      ? item.trackingId.slice(0, 8).toUpperCase()
      : "—";
    const qty = item.quantity ? `${item.quantity} tonnes` : "";
    const edd = item.estimatedDeliveryDate
    ...
    return (
      ...
      >
        ...
        <View className="flex-1">
          ...
          <Text className="text-[12px] text-gray-500 mt-0.5">{qty}</Text>
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-gray-50">
      ...
      <View className="px-4 pt-2 pb-4 bg-white border-b border-gray-100">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-[13px] text-gray-400">Good day,</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="px-4 pt-4 pb-4">
          ...
          <View className="flex-row gap-3">
            ...
            <StatCard
              label="In Transit"
              value={String(overview?.inTransit ?? 0)}
              bg="#FFF7ED"
              iconColor="#F97316"
              Icon={Truck}
              onPress={() =>
        ...
        <View className="flex-row px-4 gap-3 mb-4">
          ...
          >
            <ShoppingBag size={20} color="#A7CC48" />
            <Text className="text-[12px] font-[600] text-white mt-1">
              My Orders
            </Text>
          </TouchableOpacity>
