...
const OfftakerSettings = ({ navigation }) => {
    ...
    const prefill = async () => {
      try {
        ...
        if (raw) {
          ...
          setCompanyName(userInfo?.profile?.companyName ?? "");
          setCompanyPosition(userInfo?.profile?.companyPosition ?? "");
        }
      } catch {
  ...
  const isPwdValid =
    currentPwd && newPwd.length >= 8 && newPwd === confirmPwd;

  const handleSaveProfile = async () => {
    if (!isProfileValid) return;
    setProfileLoading(true);
    try {
      await api.put("/offtaker/update-profile", {
        firstName,
        lastName,
        phoneNumber: phone,
        companyName,
      ...
      if (raw) {
        ...
        userInfo.profile.companyName = companyName;
        userInfo.profile.companyPosition = companyPosition;
          ...
          {activeTab === TAB_PROFILE ? (
            <>
              ...
              <View className="items-center mb-6">
                <View className="relative">
                  <View
                    className="w-24 h-24 rounded-full bg-[#EFF6FF] border-4 border-white items-center justify-center"
                    style={{
                      shadowColor: "#000",
            ...
            <>
              ...
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: isPwdValid ? "#A7CC48" : "#E5E7EB",
                }}
...
export default OfftakerSettings;
